                                ...
                                <tr class="text-gray-700 dark:text-gray-400">
                                    ...
                                    <td class="px-4 py-3 text-sm">
                                        <div class="flex items-center space-x-4 text-sm">
                                            ...
                                            @can('delete', $depense)
                                                <form
                                                    action="{{ route('colocations.depenses.destroy',[$colocation, $depense]) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button
                ...
                <div class="p-4 bg-gray-900 rounded-lg shadow-xs text-white">
                        ...
                        @foreach($memebres as $memebre)
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-8 h-8 rounded bg-gray-700 flex items-center justify-center text-xs font-bold">
                                ...
                                <span
                                    class="text-sm font-bold text-green-400">{{ $memebre->user->reputation_score ?? 0 }}  </span>
                            </div>
                        @endforeach
                    ...
                    <div class="mt-5">
                        <a href="{{ route('invitations.invite', $colocation) }}"
                           class="w-full  py-2 text-xs font-semibold bg-gray-800 border border-gray-700 rounded hover:bg-gray-700 transition-colors">
                            + Inviter un membre
                        </a>
                    </div>
                </div>
